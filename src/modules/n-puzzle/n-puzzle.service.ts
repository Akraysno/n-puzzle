...
export class State {
  ...
  public getManhattanCost(): number {
    ...
    /*for (let i: number = 0 ; i < this.arr.length ; ++i) {
      ...
      v = v - 1;
      let gx: number = v % this.numRowsOrCols
      let gy: number = v / this.numRowsOrCols

      let x: number = i % this.numRowsOrCols;
      let y: number = i / this.numRowsOrCols;
  ...
  async search() {
    ...
    while (openlist.count() > 0 && !solved) {
      ...
      if (this.goal.equals(current.state, this.goal)) {
        ...
        console.log(`Solution found..\nTotal moves needed : ${solution.length}`);
