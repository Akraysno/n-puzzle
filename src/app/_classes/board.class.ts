...
export class Board {
    static checkPuzzle(startState: number[], finalState: number[], size: number): Observable<boolean> {
    ...
    static validateInversions(boardSize: number, board: number[], finalBoard: number[]) {
        let start: number[][] = Utils.chunkArray(board, boardSize)
        let final: number[][] = Utils.chunkArray(finalBoard, boardSize)
        const findD = () => {
            let xi: number
            let yi: number
            let xf: number
    ...
    static generateFinalBoard(size: number, type: NPuzzleFinalState = NPuzzleFinalState.SPIRAL): number[] {
        let final: number[][] = []
        if (type === NPuzzleFinalState.LINE) {
            let len = Math.pow(size, 2)
            let finalBoard: number[] = Array(len).fill(0, 0, len).map((v, i) => (i + 1) === len ? 0 : (i + 1))
            final = Utils.chunkArray(finalBoard, size)
        ...
        return Utils.flattenArray(final)
    }
}